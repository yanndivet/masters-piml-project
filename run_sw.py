...
def regularised_divergence(hyperparameters, key, target_observations, number_systems):
    ...
    print("estimated_observations:", estimated_observations)
    print("target_observations:", target_observations)
...
def run_sw(number_systems=cs.N_SYSTEMS):
    solver = optax.adam(learning_rate=1e-3)  # Slightly lower learning rate
    alpha = cs.INITIAL_HYPERPARAMETERS
    opt_state = solver.init(alpha)
    losses = []
    ...
    def loss_fn(parameters):
        # Force tau parameters positive before computing loss
        fixed_params = parameters.at[2:].set(jnp.maximum(parameters[2:], 1e-6))
        return regularised_divergence(fixed_params, key_regularised_divergence, target_observations_from_sampled_params, number_systems)
    ...
    for iteration in range(4001):
        ...
        if len(losses) > 2 and abs(losses[-1] - losses[-2]) < cs.EPSILON:
            break
